...
Pnet::Pnet(){
    ...
    string filename = "/home/einstein/MTCNN-light/Pnet.txt";
...
Rnet::Rnet(){
    ...
    string filename = "/home/einstein/MTCNN-light/Rnet.txt";
    readData(filename, dataNumber, pointTeam);
...
Onet::Onet(){
    ...
    string filename = "/home/einstein/MTCNN-light/Onet.txt";
    readData(filename, dataNumber, pointTeam);
...
void mtcnn::findFace(Mat &image){
    ...
    cv::FileStorage fs("/home/einstein/opencv_3/build-camera_calib-Unnamed-Debug/calib.xml", cv::FileStorage::READ);
    ...
    objectPoints.push_back(cv::Point3f(22, 0, 0));
    objectPoints.push_back(cv::Point3f(22, 22, 0));
    objectPoints.push_back(cv::Point3f(0, 22, 0));
    ...
    int x1 = (*its).y1, y1= (*its).x1;
    int x2 = (*its).y2, y2= (*its).x2;
    ...
    std::cout << "x 角度 =  " << theta_x << std::endl;
    std::cout << "y 角度 =  " << theta_y << std::endl;
    std::cout << "z 角度 =  " << theta_z << std::endl <<endl;
    ...
    if(width/distance < 3.2)
        putText(image,"[very good]",Point(50,60),FONT_HERSHEY_SIMPLEX,1,Scalar(0,0,255),4,8);//在图片上写文字
    else
        putText(image,"[Give me a smile]",Point(50,60),FONT_HERSHEY_SIMPLEX,1,Scalar(0,0,255),4,8);//在图片上写文字
    firstBbox_.clear();
